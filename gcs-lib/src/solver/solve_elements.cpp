...
bool EquationSet::Compare::operator() (const EquationSet& a, const EquationSet& b) {
...
bool EquationSet::operator== (const EquationSet& other) const {
	...
	for (
		;
		it_a != this->equations.end(), it_b != other.equations.end(); 
		++it_a, ++it_b
	) {
		if (*it_a != *it_b) {
			return false;
...
void EquationSet::set_solved() {
	// at the end, a variable will point to equations that require it in order to be solved
	// and an equation will point to the variables it solves for
...
size_t std::hash<GCS::EquationSet>::operator() (const GCS::EquationSet& eqn_set) const {
	// references:
	...
	for (auto& eqn : eqn_set.equations) {
		seed ^= reinterpret_cast<size_t>(eqn) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
	}
	return seed;
};
