...
struct func##_functor {                         \
    ...
    template <typename T>                       \
    bool operator() (                           \
        BOOST_PP_ENUM_PARAMS(n, const T* x)     \
        BOOST_PP_COMMA_IF(n)                    \
        T* r                                    \
    ) const {                                   \
...
ceres::CostFunction* create_scalar_autodiff(Functor* functor) {
    using param_list = metal::repeat<metal::number<1>, metal::number<Functor::num_params + 1>>;
